...
class FarmAutomationService : Service() {
    ...
    private fun verifyRaidDispatch() {
        ...
        val js = """
            (() => {
                ...
                return JSON.stringify({total, wrappers});
            })();
        """.trimIndent()
        webView?.evaluateJavascript(js) { raw ->
            val result = raw.orEmpty().trim('"').replace("\\\"", "\"")
            ...
            if (current > raidCountBeforeStartAll) {
                val sent = current - raidCountBeforeStartAll
                logEvent("Hasil Send All Farm: $sent raid terkirim (being raided $raidCountBeforeStartAll → $current)")
                if (resourceBuilderEnabled) startResourceBuilderCycle() else scheduleNextRandomRun()
            } else if (raidVerificationAttempt < 2 && wrappers > 0) {
                raidVerificationAttempt++
                handler.postDelayed({ verifyRaidDispatch() }, 1500)
            } else {
                logEvent("Hasil Send All Farm: 0 raid terkirim terkonfirmasi ($raidCountBeforeStartAll → $current); mencoba Start per Farm List")
                fallbackSequentialFarmListSend()
            }
        }
    }
    ...
    private fun fallbackSequentialFarmListSend() {
        if (!running) return
        updateNotification("Farm Assistant — mengirim Farm List")
        val js = """
            (() => {
                const visible = el => {
                    if (!el) return false;
        ...
        webView?.evaluateJavascript(js) { raw ->
            ...
            logEvent("Fallback Start per Farm List: $clicked tombol diklik")
            handler.postDelayed({
                val check = """
                    (() => {
                        let total=0;
                        for (const el of document.querySelectorAll('#rallyPointFarmList .farmListStatus, .farmListStatus')) {
                            const m=(el.textContent||'').replace(/\s+/g,' ').match(/(\d+)\s*\/\s*(\d+)/);
                            if(m) total+=parseInt(m[1],10);
                        }
                        return String(total);
                    })();
                ...
                webView?.evaluateJavascript(check) { value ->
                    val current = value.orEmpty().trim('"').toIntOrNull() ?: 0
                    val sent = (current - raidCountBeforeStartAll).coerceAtLeast(0)
                    logEvent("Hasil Send All Farm: $sent raid terkirim setelah fallback (being raided $raidCountBeforeStartAll → $current)")
                    if (resourceBuilderEnabled) startResourceBuilderCycle() else scheduleNextRandomRun()
                }
            }, 2500)
        }
    ...
    private fun processResourceBuilderVillage() {
        ...
        webView?.loadUrl("$server/dorf1.php?newdid=$villageId")
    }
            ...
            when {
                ...
                result.contains("candidate") -> {
                    ...
                    if (id == null) {
                        ...
                        webView?.loadUrl("$server/build.php?id=$id")
                    }
                }
        ...
        webView?.evaluateJavascript(js) { raw ->
            ...
            when {
                ...
                costs.size >= 4 && deficit.size >= 4 -> {
                    ...
                    webView?.loadUrl("$server/hero/inventory")
                }
                else -> {
        ...
        webView?.evaluateJavascript(js) { raw ->
            ...
            when {
                result == "confirmed" -> {
                    ...
                    handler.postDelayed({
                        webView?.loadUrl(pendingUpgradeUrl)
                    }, 900)
                }
    ...
    private fun discoverVillagesForBuilder() {
        ...
        val js = """
            (() => {
                ...
                const seen = new Set();
                const add = value => {
                    const m = String(value || '').match(/(?:newdid|did|villageId|village_id)[=:'\" ]+(\d+)/i) || String(value || '').match(/newdid=(\d+)/i);
                    const id = m ? m[1] : null;
                ...
                for (const root of roots) {
                    for (const a of root.querySelectorAll('a[href*="newdid="]')) add(a.getAttribute('href'));
                    for (const el of root.querySelectorAll('[data-did],[data-village-id],[data-villageid],[data-village-id]')) {
                        add(el.getAttribute('data-did') || el.getAttribute('data-village-id') || el.getAttribute('data-villageid'));
                    }
                }
                // Some current layouts expose the village switcher as a list of links without the old sidebar id.
                for (const a of document.querySelectorAll('a[href]')) {
                    const href = a.getAttribute('href') || '';
                    if (/newdid=\d+/i.test(href)) add(href);
                }
                // Include the currently active village if it is visible in the page URL.
        ...
        webView?.evaluateJavascript(js) { raw ->
            ...
            val ids = Regex("\\d+").findAll(decoded).map { it.value }.toList().distinct()
            if (ids.isEmpty()) {
                if (builderAttempt < 5) {
                    builderAttempt++
                    logEvent("Resource Builder: daftar village belum terbaca, retry ${builderAttempt}/5")
                    handler.postDelayed({ discoverVillagesForBuilder() }, 1200)
                } else {
                    logEvent("Resource Builder: daftar village tidak ditemukan setelah 5 percobaan")
                    finishResourceBuilderCycle()
                }
            } else {
                builderVillageIds = ids.toMutableList()
                builderAttempt = 0
                logEvent("Resource Builder: ${ids.size} village ditemukan (${ids.joinToString(",")})")
    ...
    private fun acceptCookiesIfPresent(done: (String) -> Unit) {
        ...
        """.trimIndent()
        webView?.evaluateJavascript(js) { raw -> done(raw.orEmpty().trim('"').lowercase(Locale.US)) }
    }
